...
export const addSlot = createAsyncThunk('slots/addSlot', async ({ floor, numSlots, token }) => {
  const res = await api.post('/api/slots', { floor, numSlots }, {
    headers: { Authorization: `Bearer ${token}` },
  });
  return res.data;
});
    ...
    builder
      ...
      .addCase(addSlot.fulfilled, (state, action) => {
        if (action.payload.slots) {
          state.list.push(...action.payload.slots);
        } else {
